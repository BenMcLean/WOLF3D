...
public class RNG : System.Random
{
	...
	private static readonly Random localRNG = new();
	...
	public int NextBits(int bits)
	{
		ulong s = StateA += 0xC6BC279692B5C323UL,
			z = (s ^ s >> 31) * (StateB += 0x9E3779B97F4A7C16UL);
	...
	public override void NextBytes(byte[] bytes)
	{
		...
		{
			n = Math.Min(i, 8);
			for (ulong bits = NextULong(); n-- != 0; bits >>= 8)
				bytes[--i] = (byte)bits;
		}
	}
	...
	public override bool Equals(object obj) =>
		obj is not null &&
		GetType() == obj.GetType() &&
		StateA == ((RNG)obj).StateA &&
		StateB == ((RNG)obj).StateB;
