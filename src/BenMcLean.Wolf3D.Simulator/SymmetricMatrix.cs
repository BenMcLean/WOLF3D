...
/// Wolfenstein 3-D uses this data structure to keep track of enemy hearing propagation between the different rooms (floor codes) of a level.
/// </summary>
public class SymmetricMatrix
{
	...
	public string ToString(string separator, string rowSeparator = null) =>
		string.Join(rowSeparator ?? separator, Data.Select(row => string.Join(separator, row)));
	public XElement Save()
	{
		XElement e = new(XName.Get(GetType().Name));
