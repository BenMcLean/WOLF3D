...
/// <summary>
/// MIDI playback engine for Super 3D Noah's Ark.
/// Interprets MIDI events and translates them to Adlib OPL2 register writes in real-time.
/// Based on ID_SD.C:2029-2433 from Super 3D Noah's Ark source code.
/// </summary>
public class MidiSignaller : IAdlibSignaller
{
	...
	private uint deltaTimeRemaining;                // ID_SD.C:2417 midiDeltaTime (longword)
	...
	{
		new byte[] { 0x21, 0x31, 0x4f, 0x00, 0xf2, 0xd2, 0x52, 0x73, 0x00, 0x00, 0x06 }, // 0
		new byte[] { 0x01, 0x31, 0x4f, 0x04, 0xf0, 0x90, 0xff, 0x0f, 0x00, 0x00, 0x06 }, // 1
		new byte[] { 0x31, 0x22, 0x10, 0x04, 0x83, 0xf4, 0x9f, 0x78, 0x00, 0x00, 0x0a }, // 2
		new byte[] { 0x11, 0x31, 0x05, 0x00, 0xf9, 0xf1, 0x25, 0x34, 0x00, 0x00, 0x0a }, // 3
		new byte[] { 0x31, 0x61, 0x1c, 0x80, 0x41, 0x92, 0x0b, 0x3b, 0x00, 0x00, 0x0e }, // 4
		new byte[] { 0x21, 0x21, 0x19, 0x80, 0x43, 0x85, 0x8c, 0x2f, 0x00, 0x00, 0x0c }, // 5
		new byte[] { 0x21, 0x24, 0x94, 0x05, 0xf0, 0x90, 0x09, 0x0a, 0x00, 0x00, 0x0a }, // 6
		new byte[] { 0x21, 0xa2, 0x83, 0x8d, 0x74, 0x65, 0x17, 0x17, 0x00, 0x00, 0x07 }, // 7
		new byte[] { 0x01, 0x01, 0x00, 0x00, 0xff, 0xff, 0x07, 0x07, 0x00, 0x00, 0x07 }, // 8
		new byte[] { 0x10, 0x00, 0x00, 0x00, 0xd8, 0x87, 0x4a, 0x3c, 0x00, 0x00, 0x00 }, // 9 (drums)
		new byte[] { 0x00, 0x00, 0x11, 0x11, 0xfa, 0xfa, 0xb5, 0xb5, 0x00, 0x00, 0x00 }, // 10
		new byte[] { 0x00, 0x00, 0x00, 0x00, 0xf8, 0xf8, 0x88, 0xb5, 0x00, 0x00, 0x00 }, // 11
		new byte[] { 0x15, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 }, // 12
		new byte[] { 0x21, 0x11, 0x4c, 0x00, 0xf1, 0xf2, 0x63, 0x72, 0x00, 0x00, 0xc0 }, // 13
	};
	...
	public void Init(IOpl opl)
	{
		...
		for (int channel = 0; channel < 9; channel++)  // Use int for C# loops (DATA_TYPES.md)
	...
	public uint Update(IOpl opl)
	{
		...
		{
			MidiEvent evt = midi.Events[eventIndex];
	...
	private void ProcessEvent(IOpl opl, MidiEvent evt)
	{
		switch (evt)
		{
			...
			case ControllerChangeEvent controllerChange:
	...
	private uint ConvertDeltaTimeTo700HzTicks(uint deltaTime)
	{
		...
		double ticks = deltaTime * timeScale;
	...
	private void ProgramChange(IOpl opl, byte channel, byte program)
	{
		...
		if (channel >= 9)
			return; // Only 9 melodic channels
