...
public partial class AutomapRenderer : Control
{
	...
	private Color _floorColor = new(0.12f, 0.12f, 0.12f),
		_ceilingColor = new(0.08f, 0.08f, 0.08f);
	...
	private int _mapPixelWidth,
		_mapPixelHeight,
		_mapWidth;   // tile count, needed to index into _automapData
		...
		for (int dy = 0; dy < TilePixels; dy++)
			for (int dx = 0; dx < TilePixels; dx++)
			{
				int srcX = (int)((dx + 0.5f) * scale),
					srcY = (int)((dy + 0.5f) * scale),
					srcOffset = (srcY * pageSqrt + srcX) << 2;
	...
	private void RebuildComposite()
	{
		...
		for (int i = 0; i < tileCount; i++)
		{
			...
			bool visible = i < _fogCurrentlyVisible.Length && _fogCurrentlyVisible[i],
				seen = i < _fogEverSeen.Length && _fogEverSeen[i];
			if (visible)
		...
		{
			byte[][] vgaGraphTiles = SharedAssetManager.CurrentGame?.VgaGraph?.Tiles,
				vswapPages = SharedAssetManager.CurrentGame?.VSwap?.Pages;
			int pageSqrt = SharedAssetManager.CurrentGame?.VSwap?.TileSqrt ?? 64;
			foreach (((ushort pwX, ushort pwY), ushort tileOrPage) in _pushWallTextures)
			{
				...
				bool visible = idx < _fogCurrentlyVisible.Length && _fogCurrentlyVisible[idx],
					seen = idx < _fogEverSeen.Length && _fogEverSeen[idx];
				if (!seen) continue;
	...
	#endregion Background baking
	#region Public update API — called by AutomapController
	/// <summary>
	...
	#endregion Public update API — called by AutomapController
	#region Rendering
	public override void _Draw()
	{
		...
		int drawW = Math.Min(ViewWidth, _mapPixelWidth - viewX),
			drawH = Math.Min(ViewHeight, _mapPixelHeight - viewY);
		if (drawW > 0 && drawH > 0)
		...
		int screenX = _playerTileX * TilePixels - viewX,
			screenY = _playerTileY * TilePixels - viewY;
