...
/// pushwalls, and billboards with pixel-level transparency detection.
...
public class PixelPerfectAiming
{
	...
	/// </summary>
	public readonly struct AimHitResult
	{
	...
	/// <summary>
	/// Performs a raycast from the specified origin in the specified direction,
	/// checking walls, doors, pushwalls, and billboards with pixel-perfect transparency detection.
	/// </summary>
	/// <param name="rayOrigin">Starting point of the ray in world coordinates</param>
	/// <param name="rayDirection">Direction of the ray (will be normalized)</param>
	/// <param name="cameraForward">Camera forward direction for billboard orientation calculations</param>
	/// <returns>Hit result containing position, distance, type, and actor index if applicable</returns>
	...
	private RayHit RaycastWalls(Vector3 rayOrigin, Vector3 rayDirection)
	{
		...
		float currentX = rayOrigin.X / Constants.TileWidth,
			currentZ = rayOrigin.Z / Constants.TileWidth;
		// DDA algorithm setup
		...
		float deltaDistX = rayDirection.X == 0 ? float.MaxValue : Mathf.Abs(1f / rayDirection.X),
			deltaDistZ = rayDirection.Z == 0 ? float.MaxValue : Mathf.Abs(1f / rayDirection.Z);
		int stepX, stepZ;
		...
		while (!hit && steps < maxSteps)
		{
			...
			float traveledDist = hitVertical ? sideDistX : sideDistZ,
				rayY = rayOrigin.Y + rayDirection.Y * traveledDist;
			if (rayY < 0 || rayY > Constants.TileHeight ||
				// Ray is outside playable vertical space, can't hit anything
				// Check if we're out of bounds
				mapX < 0 || mapZ < 0 || mapX >= _actionStage.MapAnalysis.Width || mapZ >= _actionStage.MapAnalysis.Depth)
				break;
			...
			if (!_actionStage.MapAnalysis.IsTransparent(mapX, mapZ))
				hit = true;
		}
		if (!hit)
	...
	{
		if (_actionStage.Doors is null || _actionStage.SimulatorController?.Doors is null)
			return new RayHit { IsHit = false };
		...
		for (int i = 0; i < _actionStage.SimulatorController.Doors.Count; i++)
		{
			Simulator.Entities.Door door = _actionStage.SimulatorController.Doors[i];
			...
			float doorWorldX = door.TileX.ToMetersCentered(),
				doorWorldZ = door.TileY.ToMetersCentered();
	...
	{
		if (_actionStage.SimulatorController?.PushWalls is null)
			return new RayHit { IsHit = false };
		...
		for (int i = 0; i < _actionStage.SimulatorController.PushWalls.Count; i++)
		{
			Simulator.Entities.PushWall pushWall = _actionStage.SimulatorController.PushWalls[i];
			// Get pushwall's current position in world coordinates (from 16.16 fixed-point)
			float pushWallX = pushWall.X.ToMeters(),
				pushWallZ = pushWall.Y.ToMeters();
			...
			[
				...
				RaycastPushWallFace(rayOrigin, rayDirection, pushWallX + Constants.HalfTileWidth, pushWallZ, true, Constants.TileHeight),
				// West face (negative X)
				RaycastPushWallFace(rayOrigin, rayDirection, pushWallX - Constants.HalfTileWidth, pushWallZ, true, Constants.TileHeight),
				// South face (positive Z)
				RaycastPushWallFace(rayOrigin, rayDirection, pushWallX, pushWallZ + Constants.HalfTileWidth, false, Constants.TileHeight),
	...
	/// </summary>
	private RayHit RaycastBillboards(Vector3 rayOrigin, Vector3 rayDirection, Vector3 cameraForward, float maxDistance)
	{
		...
		// Check actors
		closestHit = CheckBillboardCollection(_actionStage.Actors, rayOrigin, rayDirection, cameraForward, closestHit, HitType.Actor);
		// Check fixtures
		closestHit = CheckBillboardCollection(_actionStage.Fixtures, rayOrigin, rayDirection, cameraForward, closestHit, HitType.Fixture);
		// Check bonuses
		closestHit = CheckBillboardCollection(_actionStage.Bonuses, rayOrigin, rayDirection, cameraForward, closestHit, HitType.Bonus);
		return closestHit;
	}
	/// <summary>
	/// Checks a collection of billboards for ray intersections with pixel-perfect transparency.
	/// </summary>
	private RayHit CheckBillboardCollection(Node3D billboardParent, Vector3 rayOrigin, Vector3 rayDirection, Vector3 cameraForward, RayHit currentClosest, HitType hitType)
	{
		if (billboardParent is null)
