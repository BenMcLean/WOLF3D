...
public partial class AimIndicator : Node3D
{
	...
	public override void _Process(double delta)
	{
		Vector3 cameraForward = -_displayMode.Camera.GlobalTransform.Basis.Z,
			headPosition = _displayMode.Camera.GlobalPosition,
			// Hand 0 (right in VR, camera in flatscreen)
			origin0 = _displayMode.IsVRActive
	...
	private bool IsControllerAccessible(Vector3 controllerPosition, Vector3 headPosition) =>
		!_displayMode.IsVRActive ||
		(!(controllerPosition.Y < 0f || controllerPosition.Y > Constants.TileHeight) &&
		 _aiming.HasClearPath(headPosition, controllerPosition));
	/// <summary>
	...
	private void UpdateAimPoint(MeshInstance3D aimPoint, PixelPerfectAiming.AimHitResult hit, int handIndex)
	{
		...
		Vector3 normal = hit.Normal,
			...
			position = hit.Position + normal * Constants.PixelWidth;
		...
		Vector3 handUp = handNode.GlobalTransform.Basis.Y,
			crosshairUp = handUp - handUp.Dot(normal) * normal;
		...
		aimPoint.GlobalTransform = new Transform3D(
			basis: new Basis(crosshairRight, crosshairUp, normal),
			origin: position);
